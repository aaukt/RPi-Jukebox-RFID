...
# This script saves or restores the SINGLE status in a playlist (=folder) and enables/disables single mode according to the folder.conf of the current folder/playlist
# Usage:
...
single_check)
    ...
    ;;
enablesingle)
        if [ "$DEBUG" == "true" ]; then echo "  entering: enablesingle" >> $PATHDATA/../logs/debug.log; fi
    ...
    ;;
disablesingle)
        if [ "$DEBUG" == "true" ]; then echo "  entering: disablesingle" >> $PATHDATA/../logs/debug.log; fi
        # set the vars we need to change
        SINGLE="OFF"
        # now calling a script which will only replace these new vars in folder.conf
        # (see script for details)
